                ...
                <View style={styles.beneficiaresGroupItems}>
                        ...
                        {beneficiaires.length > 3 ? <View style={[styles.beneficiaresGroupItem, { marginLeft: -10, backgroundColor: "#ddd" }]}>
                                <View style={{ width: '80%', height: '80%', justifyContent: 'center', alignItems: 'center', backgroundColor: COLORS.primary, borderRadius: 30 }}>
                                        <Text style={{ fontWeight: 'bold', fontSize: 12, color: '#fff' }}>+{beneficiaires.length - 3}</Text>
                                </View>
                        </View> : null}
                </View>
        )
...
export const NewEmployeBeneficiairesRender = ({ selectedNewEmploye }) => {

        if (selectedNewEmploye.length == 0) return <Text style={styles.beneficiareValue}>-</Text>
        if (selectedNewEmploye.length == 1) return <Text style={styles.beneficiareValue}>{selectedNewEmploye[0].nom} {selectedNewEmploye[0].prenom}</Text>
        const items = selectedNewEmploye.slice(0, 3)
        return (
                <View style={styles.beneficiaresGroupItems}>
                        {items.map((beneficiaire, index) => {
                                return (
                                        <View style={[styles.beneficiaresGroupItem, { marginLeft: index > 0 ? -10 : 0 }]} key={index}>
                                                <Image source={ require("../../../assets/images/user.png")} style={styles.beneficiaresGroupItemImage} />
                                        </View>
                                )
                        })}
                        {selectedNewEmploye.length > 3 ? <View style={[styles.beneficiaresGroupItem, { marginLeft: -10, backgroundColor: "#ddd" }]}>
                                <View style={{ width: '80%', height: '80%', justifyContent: 'center', alignItems: 'center', backgroundColor: COLORS.primary, borderRadius: 30 }}>
                                        <Text style={{ fontWeight: 'bold', fontSize: 12, color: '#fff' }}>+{selectedNewEmploye.length - 3} nn</Text>
                                </View>
                        </View> : null}
                </View>
        )
...
export default function CorporatesScreen() {
        ...
        const { type = 'all', selectedCorporate, employee, selectNewDemandeur } = route.params || {}

        const [shosenCorporate, setShosenCorporate] = useState(null)
        const [selectedDemandeur, setSlectedDemandeur] = useState(null)
        const [selectedAllNewEmploye, setSelectedAllNewEmploye] = useState(employee || [])
        const [newDemandeur, setNewDemandeur] = useState(selectNewDemandeur || null)
        ...
        const handleSubmit = () => {
                navigation.navigate("ConfirmHailingScreen", {
                        ...
                        newEmployeCorp:selectedAllNewEmploye,
                        newDemandeur
                })
        }
        ...
        useEffect(() => {
                const { selectedBeneficiaires, selectedDemandeur, selectedNewEmploye, newDemandeur: incNewDemandeur } = route.params || {}
                ...
                if (selectedNewEmploye) {
                        setSelectedAllNewEmploye(selectedNewEmploye)
                }
                if(incNewDemandeur) {
                              setSlectedDemandeur(null)
                              if(incNewDemandeur == 'deleted') {
                                             setNewDemandeur(null)
                              } else {       
        ...
        const selectedBenef = () => {
                navigation.navigate('BeneficiaireSelectedScreen', {
                        ...
                        selectedAllNewEmploye: selectedAllNewEmploye
                })
        ...
        const isValid = () => {
               return (selectedDemandeur || newDemandeur) && (beneficiaireBenef.length > 0 || selectedAllNewEmploye.length  > 0)
        }
                        ...
                        <View style={styles.container}>
                                <View style={styles.header}>
                                        <View style={styles.exiitSearch}>
                                                ...
                                                {isInSearch ? <TextInput autoFocus style={styles.searchInput} value={q} onChangeText={n => setQ(n)} returnKeyType='search' /> : <Text style={styles.selectIndicator}>
                                                        {intl.formatMessage({ id: 'CorporatesScreen.Choisir' })}
                                                </Text>}
                                        </View>
                                ...
                                {loading ? <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                                        ...
                                        <FlatList
                                                ...
                                                renderItem={({ item: corporate, index }) => {
                                                            const isSelected = () => {
                                                                           return shosenCorporate && shosenCorporate.ID_CORP_CORPORATE == corporate.ID_CORP_CORPORATE && (selectedDemandeur|| newDemandeur) && (beneficiaireBenef.length > 0 || selectedAllNewEmploye.length > 0)
                                                            }
                                                        return (
                                                                <TouchableNativeFeedback onPress={() => handleCorporatePress(corporate)}>
                                                                        <View style={{ backgroundColor: shosenCorporate?.ID_CORP_CORPORATE == corporate.ID_CORP_CORPORATE ? '#f1f1f1' : '#fff', borderRadius: 8 }}>
                                                                                <View style={styles.institution}>
                                                                                        ...
                                                                                        {isSelected() ? <MaterialCommunityIcons name="radiobox-marked" size={24} color="#007bff" /> :
                                                                                        ...
                                                                                        <View style={styles.beneficiaresSection}>
                                                                                                ...
                                                                                                >
                                                                                                        ...
                                                                                                        <View style={styles.beneficiareDetail}>
                                                                                                                <Text style={styles.beneficiareLabel}>{intl.formatMessage({ id: 'CorporatesScreen.Demandeur' })}</Text>
                                                                                                                <Text style={styles.beneficiareValue}>
                                                                                                                        {selectedDemandeur ? 
                                                                                                                                       `${selectedDemandeur.NOM} ${selectedDemandeur.PRENOM ? selectedDemandeur.PRENOM : ''}`
                                                                                                                        : newDemandeur ? `${newDemandeur.nom} ${newDemandeur.prenom}` : '-'}
                                                                                                         </Text>
                                                                                                        </View>
                                                                                                </TouchableOpacity>
                                                                                                ...
                                                                                                >
                                                                                                        ...
                                                                                                        <View style={styles.beneficiareDetail}>
                                                                                                                <Text style={styles.beneficiareLabel}>{intl.formatMessage({ id: 'CorporatesScreen.Bénéficiaires' })}</Text>
                                                                                                                <BeneficiairesRender beneficiaires={[...beneficiaireBenef, ...selectedAllNewEmploye.map(c => ({ NOM: c.nom, PRENOM: c.prenom }))]} />
                                ...
                                <View style={{ padding: 10 }}>
                                        <TouchableNativeFeedback onPress={() => handleSubmit()} disabled={!isValid()}>
                                                <View style={[styles.submitBtn, { opacity: (!isValid()) ? 0.5 : 1 }]}>
